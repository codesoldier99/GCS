...
import { bearing, haversine, destination } from '../../util/geo'
      ...
      >
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 14, fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {selected != null ? `航点 ${selected}` : name}
          </div>
          <div style={{ fontSize: 11, color: 'var(--text-lo)' }}>
            {selected != null ? '单航点参数' : `共 ${count} 个航点`}
          </div>
        </div>
      </div>

      <div style={{ flex: 1, overflowY: 'auto', padding: 14 }}>
        {selected != null ? <WaypointEditor seq={selected} /> : <RouteSettings />}
      </div>
...
function WaypointEditor({ seq }: { seq: number }): JSX.Element {
  ...
  return (
    <>
      ...
      <Field label="高度" hint={`安全限位 ${LIMITS.altMin}–${LIMITS.altMax}m`}>
